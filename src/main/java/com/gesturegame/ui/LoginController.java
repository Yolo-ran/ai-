...
/**
 * ENTER gesture verification screen.
 *
 * <p>The animation is deliberately activated only after the account scene has
 * completed, so hidden-scene construction never consumes its opening frames.
 */
public final class LoginController {
    ...
    /** Starts the ENTER animation from frame zero once the scene is visible. */
    ...
    public void handleAgentCommand(GestureCommand command, double confidence, String hand) {
        ...
        entering = true;
        Platform.runLater(() -> {
            if (!AppStateManager.STATE_LOGIN.equals(
                    AppStateManager.getInstance().getCurrentState())) {
                entering = false;
                return;
            }
    ...
    /** ENTER does not display or decode camera frames. */
    public void updateCameraStream(String base64Image) {
        // Kept for compatibility with the gesture stream API.
    }

    /** ENTER does not display or decode camera frames. */
    public void updateCameraImage(Image image) {
        // Kept for compatibility with the local debug API.
    }
            ...
            animationTimer = new AnimationTimer() {
                ...
                public void handle(long now) {
                    double elapsedSeconds =
                            (now - animationStartNanos) / 1_000_000_000.0;
