...
```
核心数据：handX, handY（瞄准）+ gesture（FIST=单次发射）

玩法：
  - 彩球链沿螺旋轨道不断向神庙入口推进
  - 手掌只需在摄像头中央舒适区小幅移动，即可映射到全场瞄准方向
  - 稳定握拳短暂保持后发射一颗彩球，发射后自动装填另一种颜色
  - 插入后形成3颗及以上同色球即可消除，并可触发连锁得分
  - 彩球进入轨道终点则本局结束；目标模式达到指定分数获胜

实现要点：
  - PathPoint采样螺旋轨道，并按轨道距离定位每颗彩球
  - 采用角度平滑和邻近彩球轻量吸附；握拳确认期间锁定枪口，避免手型变化导致甩枪
  - 多个Projectile可同时按瞄准向量运动，与球链碰撞后按切线方向确定插入位置
  - 从插入点向两侧扫描同色连续区间，完成消除和连锁判定
  - 按难度调整颜色数量、推进速度、目标分数和每波球数
  - 进入游戏后设置短暂输入保护，对握拳/松开分别做连续帧确认，并允许少量识别闪断

渲染：
  - 深色遗迹背景与螺旋石质轨道
  - 中央青蛙炮台、当前球、投影到外圈轨道的准星和瞄准线
